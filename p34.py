...
def draw_grid():
    for y in range(GRID_SIZE):
        for x in range(GRID_SIZE):
            color = random.choice(BEIGE_COLORS)
            if grid[y][x] == 1:
                color = RED
            elif grid[y][x] == 2:
                color = BLUE
...
def main():
    ...
    running = True
    while running:
        player_score, _ = max(count_longest_line(1, x, y) for y in range(GRID_SIZE) for x in range(GRID_SIZE))
        ai_score, _ = max(count_longest_line(2, x, y) for y in range(GRID_SIZE) for x in range(GRID_SIZE))
