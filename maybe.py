...
def create_game_window():
    ...
    def draw_scoreboard(player_score, ai_score):
        ...
        player_score_text = font.render(f"Player: {player_score - 1}", True, RED)
        ai_score_text = font.render(f"AI: {ai_score - 1}", True, BLUE)
    ...
    def reset_game():
        nonlocal grid, game_over, winning_line, winner
        grid = [[0 for _ in range(GRID_SIZE)] for _ in range(GRID_SIZE)]
        game_over = False
        winning_line = []
        winner = None
    ...
    def main():
        nonlocal game_over, winning_line, winner
        running = True
        while running:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if y >= WINDOW_SIZE and y <= WINDOW_SIZE + SCOREBOARD_HEIGHT:
                        if WINDOW_SIZE // 2 - reset_text.get_width() // 2 <= x <= WINDOW_SIZE // 2 + reset_text.get_width() // 2:
                            reset_game()
                        elif WINDOW_SIZE // 2 - main_menu_text.get_width() // 2 <= x <= WINDOW_SIZE // 2 + main_menu_text.get_width() // 2:
                            main_menu()
